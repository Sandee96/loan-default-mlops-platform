...
Three candidate models are trained and compared on every run: Logistic Regression, Random Forest,
and Gradient Boosting. The best model is selected by ROC-AUC. Current results (see `models/metrics.json`
for exact, live figures — not reproduced statically here to avoid stale numbers):

- Gradient Boosting was selected as the best model in the most recent run, with ROC-AUC ~0.777 —
  narrowly ahead of Random Forest (~0.773) and Logistic Regression (~0.733). All three scores fall in a
  realistic, credible range for this dataset and problem (not overfit, not underperforming).
- Notably, Gradient Boosting achieves higher precision (~0.67) but lower recall (~0.35) than Random
  Forest (~0.50 precision, ~0.58 recall) — it is more conservative about flagging default risk, with
  fewer false alarms but more missed actual defaulters. Since model selection here uses ROC-AUC (which
  evaluates the full probability ranking rather than one fixed threshold), Gradient Boosting still comes
  out ahead overall, but in a real lending context this precision/recall trade-off would be a genuine
